...
-- Insert seed data for subjects - Fixed query
INSERT INTO public.subjects (name, school_id)
SELECT subjects_list.name, schools.id
FROM (
  VALUES 
    ('Matemáticas'),
    ('Lengua'),
    ...
    ('Educación Física'),
    ('Tecnología')
) AS subjects_list(name)
CROSS JOIN public.schools
WHERE schools.teaching_level = 'secundario';

-- Insert seed data for administrative positions - Fixed query
INSERT INTO public.administrative_positions (name, school_id)
SELECT positions_list.name, schools.id
FROM (
  VALUES
    ('Director'),
    ('Vice Director'),
