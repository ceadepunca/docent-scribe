-- Clean up existing ENET data (fixed column ambiguity)
...
WHERE subject_selection_id IN (
  SELECT iss.id FROM public.inscription_subject_selections iss
  ...
  SELECT ips.id FROM public.inscription_position_selections ips
